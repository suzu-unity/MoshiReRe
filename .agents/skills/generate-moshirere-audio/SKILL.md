...
2. In its `Text to Music (MiniMax Music 3)` node, use the RTX 4070 defaults:
   ...
   - `tiled_decode`: `true`
3. Translate the Japanese request into natural English. Write `caption` with `Global Metadata`, `Vocal Details`, and `Arrangement`; preserve mood, tempo, instruments, energy, scene, loop intent, and exclusions. For instrumental BGM, state `instrumental only, no vocals, no spoken word` and use `[Instrumental]` for lyrics.
4. Use a fixed seed supplied by the user or record the generated seed. Start an audition at 30 seconds when VRAM risk is unknown; use the template 60-second duration for a final short BGM.
5. Set `SaveAudioAdvanced.filename_prefix` to a unique short ASCII snake_case name without a directory. Keep the template MP3 output unless the user requests another supported format. ComfyUI numeric suffixes preserve variants; never overwrite a named asset without permission.
