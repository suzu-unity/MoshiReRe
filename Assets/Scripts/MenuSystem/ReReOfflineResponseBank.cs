...
public static class ReReOfflineResponseBank
{
    public static List<ReReResponseEntry> CreateEntries()
    {
        return new List<ReReResponseEntry>
        {
            ...
            Entry("papa_cafe_briefing", new[] { "店", "カフェ", "下調べ", "何を見る", "ヒント", "対象", "待ち合わせ" },
                new[] { "先に見るのは窓際の席。対象が送った写真に、席番号つきの金具が写っていたよ。", "入口から待ち合わせ席までの導線と、窓際の二人席を確認して。自慢話につながる物があるかも。", "褒め言葉を探すより、相手が『気づいてほしい物』を一つ見つけよう。" },
                ReReExpression.Thinking, requiredContextTags: new[] { "papa_cafe_briefing" }, priority: 24),
            Entry("papa_cafe_exploration", new[] { "店", "カフェ", "調べ", "探索", "ヒント", "席", "番号", "鍵", "タグ" },
                new[] { "窓際の二人席を見て。写真の鍵にあった番号と同じ物がないか確認しよう。", "まだなら窓際、見つけた後なら出口までの導線。下調べはその二つで十分だよ。", "店員の作業場には入らなくていい。客席から見える範囲だけで材料は揃うよ。" },
                ReReExpression.Listening, requiredContextTags: new[] { "papa_cafe" }, priority: 18),
            Entry("papa_cafe_key", new[] { "鍵", "キー", "タグ", "17", "アイテム", "見せる", "提示" },
                new[] { "17番のタグは、援助を迫る道具じゃなくて思い出を語ってもらう入口にしよう。", "いきなり返すより、写真にも同じ番号があったと伝えて。相手が意味を説明する余白を残してね。" },
                ReReExpression.Delighted, requiredContextTags: new[] { "papa_cafe" }, requiredInventoryIds: new[] { "old_key" }, priority: 30),
